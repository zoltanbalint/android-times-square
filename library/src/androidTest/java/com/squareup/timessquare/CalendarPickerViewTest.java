...
public class CalendarPickerViewTest {
    ...
    public void testInitDecember() throws Exception {
        ...
        assertTrue(view.months.size() == 12);
    }
    ...
    public void selectDateDoesntSelectDisabledCell() {
        ...
        assertTrue(wasAbleToSetDate);
        assertTrue(view.selectedCells.get(0).isSelectable());
    }
    ...
    public void testMultiselectWithNoInitialSelections() throws Exception {
        view.init(minDate, maxDate, locale) //
                .inMode(MULTIPLE);
        assertEquals(view.selectionMode, MULTIPLE);
        assertTrue(view.getSelectedDates().size() == 0);

        view.selectDate(minDate);
        assertTrue(view.getSelectedDates().size() == 1);

        Calendar secondSelection = buildCal(2012, NOVEMBER, 17);
        view.selectDate(secondSelection.getTime());
        assertTrue(view.getSelectedDates().size() == 20);
        assertEquals(view.getSelectedDates().get(1).getTime(), secondSelection.getTimeInMillis());
    }
    ...
    public void testWithoutDateSelectedListener() throws Exception {
        ...
        assertEquals(view.selectedCals.get(0).get(DATE), jumpToCal.get(DATE));
    }
    ...
    public void testRangeSelectionWithNoInitialSelection() throws Exception {
        view.init(minDate, maxDate, locale)
                .inMode(RANGE);
        assertTrue(view.selectedCals.size() == 0);
        assertTrue(view.selectedCells.size() == 0);
    ...
    public void testInitWithoutHighlightingCells() {
        ...
        assertTrue(view.highlightedCals.size() == 0);
        assertTrue(view.highlightedCells.size() == 0);
    }
    ...
    public void testHighlightingCells() {
        ...
        assertTrue(view.highlightedCals.size() == 1);
        assertTrue(view.highlightedCells.size() == 1);

        List<List<MonthCellDescriptor>> cells = getCells(NOVEMBER, 2012);
        assertTrue(cells.get(3).get(2).isHighlighted());
    }
    ...
    private void assertRangeSelected() {
        assertTrue(view.selectedCals.size() == 2);
        assertTrue(view.selectedCells.size() == 7);
        assertTrue(view.getSelectedDates().size() == 7);
    }

    private void assertOneDateSelected() {
        assertTrue(view.selectedCals.size() == 1);
        assertTrue(view.selectedCells.size() == 1);
        assertTrue(view.getSelectedDates().size() == 1);
    }
    ...
    public void testLocaleSetting() throws Exception {
        ...
        assertEquals(firstDay.getText().toString(), "Mo"); // Montag = Monday
        assertEquals(monthView.title.getText().toString(), "Dezember 2012");
    }
    ...
    public void testFirstDayOfWeekIsMonday() throws Exception {
        ...
        assertEquals(cal.getFirstDayOfWeek(), Calendar.MONDAY);

        view.init(minDate, maxDate, greatBritain);
        MonthView monthView = (MonthView) view.getAdapter().getView(1, null, null);
        CalendarRowView header = (CalendarRowView) monthView.grid.getChildAt(0);
        TextView firstDay = (TextView) header.getChildAt(0);
        assertEquals(firstDay.getText().toString(), "Mon"); // Monday!
    ...
    public void testSetShortWeekdays() throws Exception {
        ...
        CalendarRowView header = (CalendarRowView) monthView.grid.getChildAt(0);
        TextView firstDay = (TextView) header.getChildAt(0);
        assertEquals(firstDay.getText().toString(), "M"); // Monday!
        TextView secondDay = (TextView) header.getChildAt(1);
        assertEquals(secondDay.getText().toString(), "T"); // Tuesday!
        TextView thirdDay = (TextView) header.getChildAt(2);
        assertEquals(thirdDay.getText().toString(), "W"); // Wednesday!
    }
    ...
    private static void assertCell(List<List<MonthCellDescriptor>> cells, int row, int col,
        ...
        assertEquals(cell.getValue(), expectedVal);
        assertEquals(cell.isCurrentMonth(), expectedCurrentMonth);
        assertEquals(cell.isSelected(), expectedSelected);
        assertEquals(cell.isToday(), expectedToday);
        assertEquals(cell.isSelectable(), expectedSelectable);
        assertEquals(cell.getRangeState(), expectedRangeState);
    }
